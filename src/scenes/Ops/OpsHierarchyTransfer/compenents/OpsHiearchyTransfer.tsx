...
import { PositionOutput } from '../../../../services/opsHierarchy/dto/getAllHierarchyOutput';
...
export default function OpsHiearchyTransfer(props: IProps) {
  ...
  const { opsHierarchyStore, sourceTitle, targetTitle } = props;
  const { Option } = Select;
  ...
  async function getUnits() {
    let result = await opsHierarchyStore.getUnit();
  ...
  const setItemsSelected = async (keys) => {
    opsHierarchyStore
      ...
      .then(() => {
        opsHierarchyStore.updateSetTrue({
          ids: keys,
        });
      })
  ...
  function onUnitChange(value) {
    ...
    let positions = opsHierarchyStore.units.items.find((item) => item.id === value)?.positions;
